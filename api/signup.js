export default async function handler(req, res) {
  ...
  if (!SB_KEY) {
    return res.status(500).json({ status: 'error', message: 'Missing env var' });
  }
  ...
  // block spam email patterns — 4+ numbers in a row
  ...
  // if referred give slight boost
  if (body.referred_by && body.referred_by !== 'direct') {
    position = Math.max(1, position - 5);
  }

  // insert new user with correct position
  var insertBody = {
    email:       body.email,
    exam:        body.exam       || '',
